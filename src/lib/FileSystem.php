...
class FileSystem
{
    ...
    public function get(string $path, $lock = false)
    {
        ...
        throw new CoreException('core file not found',"File does not exist at path {$path}",null,NoticeType::PAGE_ERROR);
    }
    ...
    public function allFiles($directory, $hidden = false)
   {
     ...
     * @return bool
     */
    public function deleteEmptyDirectoriesRecursive(string $directory)
    {
        if (! $this->isDirectory($directory)) {
            return false;
        }
        $directory = UtilsTools::replaceSeparator($directory);
        $directoryArray = explode(DIRECTORY_SEPARATOR,$directory);
        for ($i=0;$i<count($directoryArray);$i++){
            $deleteDirectory = implode(DIRECTORY_SEPARATOR,$directoryArray);
            if(count($this->files($deleteDirectory))===0){
